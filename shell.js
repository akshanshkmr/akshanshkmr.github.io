...
function buildRegistry() {
     ...
     for (const cmd of [
          help, about, projects, skills, experience, contact, resume,
          clear, share, snake,
          sudo, vim, coffee, matrix, init, whoami,
     ]) {
          r.register(cmd);
     }
     return r;
...
async function main() {
     ...
     const ctx = {
          scroll, resume, registry,
          renderBlock, renderEcho, renderBox, renderInitial,
          showToast, clearScroll,
     };
     ...
     document.addEventListener('click', (e) => {
          if (e.target.closest('a, button, .project-name, .snake-overlay, .vim-modal, input')) return;
          input.focus();
     });
     input.focus();
